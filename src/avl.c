...
void avl_insere_node(tnode **parv, tnode *pai, titem item)
{
    if (*parv == NULL)
    {
    ...
    {
        avl_insere_node(&(*parv)->esq, *parv, item);
    }
    else
    {
        avl_insere_node(&(*parv)->dir, *parv, item);
    }
...
{
    avl_insere_node(parv, NULL, item);
}
